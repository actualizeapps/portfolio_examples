...
$perfTime = microtime(true);
$perfResponse = '';

$entries = [];
if (file_exists($dbFile)) {
    $fileContent = file_get_contents($dbFile);
    $perfResponse .= "file_get_contents time: " . (microtime(true) - $perfTime) . " seconds\n";
    $perfTime = microtime(true);
    if (!empty($fileContent)) {
        $entries = json_decode($fileContent, true) ?: [];
    }
    $perfResponse .= "json_decode time: " . (microtime(true) - $perfTime) . " seconds\n";
}

if ($endDate != null) {  
    $perfTime = microtime(true);
    $entries = accumulateScoresAndFilterTime($entries, $endDate);
    $perfResponse .=  "accumulateScoresAndFilterTime time: " . (microtime(true) - $perfTime) . " seconds\n";
    $perfTime = microtime(true);
    $entries = mergeSort($entries);
    $perfResponse .=  "mergeSort time: " . (microtime(true) - $perfTime) . " seconds\n";
}

$perfTime = microtime(true);
json_encode($entries);
$perfResponse .=  "json_encode time: " . (microtime(true) - $perfTime) . " seconds\n";
